...
import { parseItauStatement } from "@/lib/card-statement-parsers/itau";
import { ITAU_003_LINHAS } from "@/lib/card-statement-parsers/itau.regression.semantica";
describe("iof", () => { it("linear", () => {
  const r: any = (parseItauStatement as any)(ITAU_003_LINHAS.map((text,i)=>({y:i,text,cells:[{x:0,text}]})));
