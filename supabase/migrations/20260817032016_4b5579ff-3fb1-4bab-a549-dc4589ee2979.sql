...
CREATE TABLE IF NOT EXISTS public.bank_financial_repair_logs (
  ...
  transaction_removed jsonb NOT NULL,
  transactions_direction_corrected jsonb NOT NULL,
  transfer_group_id uuid,
  bb_transaction_preserved jsonb,
  post_validation jsonb NOT NULL,
  executed_at timestamptz NOT NULL DEFAULT now(),
  executed_by uuid,
  created_at timestamptz NOT NULL DEFAULT now()
);

GRANT SELECT ON public.bank_financial_repair_logs TO authenticated;
GRANT ALL ON public.bank_financial_repair_logs TO service_role;

ALTER TABLE public.bank_financial_repair_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Family members read financial repair logs" ON public.bank_financial_repair_logs;
CREATE POLICY "Family members read financial repair logs"
ON public.bank_financial_repair_logs FOR SELECT TO authenticated
...
BEGIN
  ...
  -- estado atual do ledger, lido do banco
  ...
  -- IDEMPOTÊNCIA: reparo já aplicado?
  ...
  -- 2. preservar o lado original: desvincular do grupo, virar movimento normal
  ...
  -- 3. remover somente a contrapartida artificial
  DELETE FROM public.transactions WHERE id = _remove_id;

  -- 4/5. corrigir a direção dos rendimentos, sem tocar em valor, data ou lineage
  UPDATE public.transactions
     SET tipo = 'ENTRADA'::transaction_type, updated_at = now()
   WHERE id = ANY(_fix_ids);

  -- 6. recalcular o ledger real
  ...
  -- 7. validar checkpoints do extrato canônico, recalculados do banco
  WITH cps AS (
    SELECT c.data, round(c.saldo, 2) AS esperado,
           round(coalesce((
             SELECT sum(CASE
